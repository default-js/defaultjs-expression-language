...
2. **Raise code quality** — fix existing defects, sharpen the structure. The pluggable executer strategy is the part of this work already begun. The freeze that held this back is **lifted (2026-08-22)**: `SPECIFICATION.md` now says what the resolver is meant to do, so a fix has something to be derived from other than the code. The places where the code and the specification disagree are listed in its section 10 and carried as `BACKLOG.md` entries. Since 2026-08-24 **every one of them is pinned by a failing test** marked `it.fails` in `test/spec/` — 63 at the start, **9 left** after the data methods of 6.6 and the chain getters of 5.5 were fixed on 2026-08-30 — so a fix is finished when its markers can be removed and the gate is still green. See `DECISIONS.md` for how that mechanism works and why.
