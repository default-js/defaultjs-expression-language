...
2. **Raise code quality** — fix existing defects, sharpen the structure. The pluggable executer strategy is the part of this work already begun. The freeze that held this back is **lifted (2026-08-22)**: `SPECIFICATION.md` now says what the resolver is meant to do, so a fix has something to be derived from other than the code. Since 2026-09-05 it is written as though every rule in it holds, so it carries no index of what is missing: **the specification is the release gate for 3.0.0**, and where the code does not keep a rule yet, `BACKLOG.md` says so and a test marks it. Since 2026-08-24 **every one of them is pinned by a failing test** marked `it.fails` — 63 at the start, **8 left** after the data methods of 6.6 and the chain getters of 5.5 were fixed on 2026-08-30 and the default executer moved on 2026-09-01 — so a fix is finished when its markers can be removed and the gate is still green. Since 2026-09-05 the marker has **one meaning per directory**, and the directory is what says which: the **6 in `test/spec/`** say *the resolver does not keep this rule yet* and are the ones this count is about, the **8 in `test/executer/capabilities/`** say *this executer does not support this*, which is neither a defect nor a rule broken. Three of them used to be counted as defects and stopped being defects that day, by decision rather than by a fix: the two cells of the global-write containment and the context value the esprima rewrite loses inside a nested function. For the containment it is `SPECIFICATION.md` 6.5 that moves, because the package cannot keep that promise under every implementation. See `DECISIONS.md` for both mechanisms.
