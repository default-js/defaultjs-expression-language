...
2. **Raise code quality** — fix existing defects, sharpen the structure. The pluggable executer strategy is the part of this work already begun. The freeze that held this back is **lifted (2026-08-22)**: `SPECIFICATION.md` now says what the resolver is meant to do, so a fix has something to be derived from other than the code. The places where the code and the specification disagree are listed in its section 10 and carried as `BACKLOG.md` entries. Since 2026-08-24 **every one of them is pinned by a failing test** marked `it.fails` in `test/spec/` — 63 of them — so a fix is finished when its markers can be removed and the gate is still green. See `DECISIONS.md` for how that mechanism works and why.
3. **Raise test coverage** — **largely done (2026-08-24).** The conformance suite of `test/spec/` pins every rule of `SPECIFICATION.md` and took coverage from 84 % to **92.51 % of statements, 90.76 % of branches**. What is left is five named items in `BACKLOG.md`, and none of them is a missing test: dead code, two debug switches, one branch that only becomes reachable when a defect is fixed, two methods whose place on the public surface is undecided, and one `catch` nothing can reach.
...
- `plans/` — one file per ordered undertaking, named after it. **`plans/expression-parsing.md` is running**, opened 2026-08-29: the matching-brace parser, escaping, the scope walk and the error policy, in the order their shared code allows. The three before it — the toolchain modernization, the specification, and the conformance suite — were retired on 2026-08-21, 2026-08-22 and 2026-08-24. A plan is a living document: update a stage's status the moment it goes green, along with what was actually installed and any deviation from the intent. When the undertaking is finished the plan is **deleted** — durable outcomes move into `DECISIONS.md` first, git history keeps the rest. A finished plan left lying around gets read as instructions. With nothing running, `plans/` does not exist.
