...
2. **Raise code quality** — fix existing defects, sharpen the structure. The pluggable executer strategy is the part of this work already begun. The freeze that held this back is **lifted (2026-08-22)**: `SPECIFICATION.md` now says what the resolver is meant to do, so a fix has something to be derived from other than the code. Since 2026-09-05 it is written as though every rule in it holds, so it carries no index of what is missing: **the specification is the release gate for 3.0.0**, and where the code does not keep a rule yet, `BACKLOG.md` says so and a test marks it. Since 2026-08-24 **every one of them is pinned by a failing test** marked `it.fails` — 63 at the start, and the count fell as the data methods of 6.6 and the chain getters of 5.5 were fixed on 2026-08-30, the default executer moved on 2026-09-01 and its write-back landed on 2026-09-07 — so a fix is finished when its markers can be removed and the gate is still green. Since 2026-09-05 the marker has **one meaning per directory**, and the directory is what says which: the **4 in `test/spec/`** say *the resolver does not keep this rule yet* — all four in `4.1-the-static-entry-points.Test.js`, so one open blocker is what is pinned that way today — while the **60 cells in `test/executer/capabilities/`** say *this executer does not support this*, which is neither a defect nor a rule broken. Counted 2026-09-20, after the write-back was removed again and six cells of `context-write` moved to `no` with it (`DECISIONS.md`, same day); the gate reports both together as 64 *expected fail*. What blocks 3.0.0 is not this count but the `Blocks 3.0.0` marker in `BACKLOG.md`. Three of them used to be counted as defects and stopped being defects that day, by decision rather than by a fix: the two cells of the global-write containment and the context value the esprima rewrite loses inside a nested function. For the containment it is `SPECIFICATION.md` 6.5 that moves, because the package cannot keep that promise under every implementation. See `DECISIONS.md` for both mechanisms.
3. **Raise test coverage** — **largely done (2026-08-24).** The conformance suite of `test/spec/` pins every rule of `SPECIFICATION.md` and took coverage from 84 % to **93.07 % of statements, 91.36 % of branches** (2026-09-07, over 737 cases; `src/ExpressionResolver.js` is at 100 % of lines). What is left is four named items in `BACKLOG.md`, and none of them is a missing test: dead code, two debug switches, one branch that only becomes reachable when a defect is fixed, and two methods whose place on the public surface is undecided.
...
- `plans/` — one file per ordered undertaking, named after it. **No plan is running.** The nine so far — the toolchain modernization, the specification, the conformance suite, the expression parsing rework, the four of 2026-09-01 that rebuilt the test suite one after the other (the executer conformance suite, the test structure, the test concept, and the resolver tested without an evaluation), and the fine-grained capability catalogue of 2026-09-05 — were retired on 2026-08-21, 2026-08-22, 2026-08-24, 2026-08-29, 2026-09-01 and 2026-09-05. What outlived the last five is in `DECISIONS.md`, `TESTING.md` and part B of `SPECIFICATION.md`. A plan is a living document: update a stage's status the moment it goes green, along with what was actually installed and any deviation from the intent. When the undertaking is finished the plan is **deleted** — durable outcomes move into `DECISIONS.md` first, git history keeps the rest. A finished plan left lying around gets read as instructions. With nothing running, `plans/` does not exist.
...
**Four things are tested, and each has one place** (2026-09-05). `test/spec/` tests the **resolver** against `TestExecuter`, the implementation the suite owns, which **evaluates nothing** — it answers the statement it was handed, so a case reads the resolver's own work out of the result. `test/executer/capabilities/` tests **what an executer supports**: how far it carries JavaScript over a dynamic context, asked of all four, each case with a row in `CAPABILITIES` (`test/ExecuterCapabilities.js`) that says `yes` or `no` — two states, because an executer has capabilities and nothing else, so nothing it does can break a rule it was never given. `test/executer/rules/` holds the **rules of the resolver that only show through a statement** — the chain walk, the name snapshot, the error policy — asked of all four as a plain `it`, no row and no state, because the proxy does that work and no implementation may decline it. `test/general/` holds what pins no rule at all. A capability file is named after its capability, every other file after its section of `SPECIFICATION.md` as `<section>-<slug>.Test.js`.
